...
1. From the installer ISO (booted, Yubikey present, SSH accessible), run a
   ...
   b. Installs a minimal NixOS base system to disk fully offline (spec 0010).
   c. On first boot, generates the host SSH key, enrolls it as a sops recipient
      (spec 0009), and decrypts secrets with the host key for all subsequent
      boots.
...
6. **Copy repo from ISO** to `/mnt/etc/nixos` — the ISO embeds the full
   set of files needed by `nixos-install`: `flake.nix`, `flake.lock`,
   `modules/`, `scripts/`, `.sops.yaml`, `install-config.yaml`,
   `enroll-host-key.sh`, and `wrapped-install-key.bin`.
   `hardware-configuration.nix` (generated in step 5) is already in place.

7. **Copy pre-built system closure to target store** — the ISO embeds the
   full store closure of the killy target system (spec 0010). The closure is
   copied from the ISO's `/nix/store` into `/mnt/nix/store` and registered
   in `/mnt`'s Nix DB via `nix-store --dump-db | nix-store --store /mnt --load-db`.

8. **Install NixOS** — re-evaluates the flake against the installed
   `hardware-configuration.nix` (real partition UUIDs). All package
   dependencies are already in `/mnt/nix/store`; only thin text-file
   derivations (systemd units, fstab) are built locally. No network access:
   ```bash
...
SSH host key generation and `sops updatekeys` (host age key enrollment) happen
on first boot via `killy-enroll-host-key.service` (spec 0009), not at install
time.

### 5.3 NixOS configuration: `killy/`

`killy/` holds the installed system's NixOS configuration (mirrors `/etc/nixos/`
on the installed machine — see spec 0008 for the full layout):

```
killy/
  flake.nix                — nixosConfigurations.killy entry (self-contained)
  configuration.nix        — top-level: imports hardware.nix + hardware-configuration.nix
  hardware-configuration.nix — generated by killy-install (NOT in git; .gitignored)
  modules/
    hardware.nix           — stable hardware declarations (kernel modules, CPU microcode)
...
```bash
nixos-rebuild switch --flake ./killy#killy --target-host fred@<killy-ip> --use-remote-sudo
```
