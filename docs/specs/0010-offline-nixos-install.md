...
`killy-install` uses a two-step approach to achieve fully offline installation
with a correct `fstab`:

**Step 1 — Copy pre-built closure to target store.**
The ISO embeds the pre-built system closure (built with a stub `fileSystems`).
`killy-install` reads the pre-built toplevel path from
`/etc/nixos/killy-system-path` (written at ISO build time), computes its full
requisite closure via `nix-store --query --requisites`, then:

1. Creates `/mnt/nix/store/` and copies all store paths into it in batches of
   50 (to stay under shell ARG_MAX limits):
   ```
   cp -a --no-preserve=ownership <batch> /mnt/nix/store/
   ```
2. Registers all paths in `/mnt`'s Nix DB in matching batches:
   ```
   nix-store --dump-db <batch> | nix-store --store /mnt --load-db
   ```

**Step 2 — Install with flake re-evaluation.**
With all package dependencies pre-loaded in `/mnt/nix/store`, run:
```
...
absolute path on the live system). `nixos-install` re-evaluates the flake
against the real `hardware-configuration.nix` (with correct partition UUIDs)
and builds only the thin `toplevel` derivation (systemd units, fstab — no
packages). All package dependencies are already in `/mnt/nix/store`; no
...
| `installer/bin/killy-install` | Step 10: `cp` + `nix-store --dump-db/load-db` to pre-populate `/mnt/nix/store`; step 11: `nixos-install --impure --no-channel-copy --flake`; `wrapped-install-key.bin` copied to `killy/` subdir |
